...
export default function SolutionScheduleByClassPage() {
  ...
  const normalizedEvents = useMemo(() => {
    ...
    return annotateOverlapsByClass(baseEvents);
  }, [rows, detectedFields]);

  const overlapSummary = useMemo(() => {
    return buildOverlapSummaryByClass(normalizedEvents);
  }, [normalizedEvents]);

  const classOptions = useMemo(() => {
    const uniqueClasses = [...new Set(normalizedEvents.map((event) => event.className).filter(Boolean))]
      .sort((a, b) => a.localeCompare(b, "pt"));

    return uniqueClasses.map((className) => {
      const totalOverlaps = overlapSummary.classTotals.get(className) || 0;

      return {
        id: className,
        label:
          totalOverlaps > 0
            ? `${className} (${totalOverlaps} sobreposição${totalOverlaps === 1 ? "" : "ões"})`
            : className,
      };
    });
  ...
  const weekOptions = useMemo(() => {
    ...
    return uniqueWeeks.map((weekKey) => {
      const classWeekKey = `${selectedClass}__${weekKey}`;
      const overlapsForClassWeek = selectedClass
        ? overlapSummary.classWeekTotals.get(classWeekKey) || 0
        : 0;
      ...
      return {
        id: weekKey,
        label:
          selectedClass && overlapsForClassWeek > 0
            ? `${baseLabel} (${overlapsForClassWeek} sobreposição${overlapsForClassWeek === 1 ? "" : "ões"})`
            : baseLabel,
      };
    });
  }, [normalizedEvents, overlapSummary, selectedClass]);
  ...
  useEffect(() => {
    if (selectedWeek && !weekOptions.some((item) => item.id === selectedWeek)) {
      setSelectedWeek(weekOptions[0]?.id || "");
    }
  }, [weekOptions, selectedWeek]);
  ...
  const filteredEvents = useMemo(() => {
    return normalizedEvents.filter((event) => {
      const matchesClass = selectedClass ? event.className === selectedClass : true;
      const matchesWeek = selectedWeek ? event.weekKey === selectedWeek : true;
      return matchesClass && matchesWeek;
    });
  }, [normalizedEvents, selectedClass, selectedWeek]);
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <h1 style={styles.title}>Horário por turma</h1>
        <p style={styles.description}>
          Visualização semanal do horário da solução, com deteção de sobreposições por turma.
        </p>
        ...
        <div style={styles.switcher}>
          ...
          <NavLink
            to={`/problems/${id}/solutions/${solutionId}/schedule/classes`}
          ...
          <NavLink
            to={`/problems/${id}/solutions/${solutionId}/schedule/courses`}
            style={styles.switcherButton}
          >
            Horário por cadeira
          </NavLink>
...
function normalizeScheduleRow(row, fields, index) {
  ...
  return {
    id: `${className}-${weekKey}-${day}-${start}-${end}-${index}`,
    room,
    className,
    day,
    start,
...
function annotateOverlapsByClass(events) {
  const groups = new Map();

  for (const event of events) {
    const key = `${event.weekKey}__${event.className}__${event.day}`;
    if (!groups.has(key)) {
      groups.set(key, []);
    }
...
function buildOverlapSummaryByClass(events) {
  const classTotals = new Map();
  const classWeekTotals = new Map();
  ...
  for (const event of events) {
    ...
    classTotals.set(event.className, (classTotals.get(event.className) || 0) + 1);

    const classWeekKey = `${event.className}__${event.weekKey}`;
    classWeekTotals.set(classWeekKey, (classWeekTotals.get(classWeekKey) || 0) + 1);
  }
