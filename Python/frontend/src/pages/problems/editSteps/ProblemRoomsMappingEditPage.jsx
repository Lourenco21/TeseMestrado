        ...
        >
          ...
          >
            {selectedValues.length > 0
              ? `${selectedValues.length} colunas selecionadas`
              : placeholder}
          </span>
...
export default function ProblemRoomsMappingStepPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const saved =
          result?.saved_rooms_mapping_data || draft?.rooms_mapping_data || {};
        ...
        setCharacteristics(
          saved?.characteristics
            ? {
                ...
                ...saved.characteristics,
                config: {
                  ...createEmptyCharacteristics(saved.characteristics.format).config,
  ...
  const linkingErrors = useMemo(() => {
    ...
    if (!linking.rooms_file_room_column) {
      errors.push("Falta selecionar a coluna do ficheiro de salas que identifica a sala.");
    }
  ...
  const characteristicsErrors = useMemo(() => {
    ...
    if (characteristics.format === "multiple_columns") {
      ...
      if (!characteristics.config?.selected_values?.length) {
        errors.push("Adiciona pelo menos um valor que indique que a característica está selecionada.");
      }
    }
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        <p style={styles.description}>
          Associe as colunas do ficheiro de salas aos campos necessários e defina
